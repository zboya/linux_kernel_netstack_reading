...
linux实现的接口是bsd套接字https://zh.wikipedia.org/wiki/Berkeley套接字
client
	sokcet() 	创建socket fd
	connect()	连接对端
	write()		发送数据
...
// 文件系统与socket结合后的操作集。
// socket fd作为特殊的fd，也拥有和普通fd一样的操作
			 ...
			 struct socket **res, int kern)
{
	...
	// https://blog.csdn.net/yangguo_2011/article/details/17144169
	// 在访问内部对象之前对内核代码进行修改，以调用一个代表实施函数的钩子，
	// 该实施函数实现安全策略。该函数根据预定义的策略验证操作能否继续进行。
	// 安全函数存储在一个安全操作结构中，该结构包含必须受到保护的基本操作。
	// 例如，security_socket_create 钩子（security_ops->socket_create）
	// 在创建新 socket 之前检查权限，并考虑协议集、类型、协议，以及 socket 是在内核中创建还是在用户空间中创建
...
{
	// current->nsproxy->net_ns 获取当前命名空间的net
...
// 其实里面做的工作，就是将系统调用的参数统一变为了使用long型来接收，再强转转为int，
// 也就是系统调用本来传下来的参数类型。那么这么强转一下究竟是为什么呢？
// 原因就是64位的Linux有一个名为CVE-2009-2009的漏洞，这个漏洞的具体内容，请大家看我的另外一篇博客：
// http://blog.csdn.net/hxmhyp/article/details/22619729
// 定义系统调用socket
		 ...
		 struct sockaddr __user *addr,  int addr_len)
{
	...
	msg.msg_namelen = 0;
	if (addr) {
		//用户空间通过套接字传输的数据内容在tcp_sendmsg函数中的skb_add_data拷贝到SKB中
